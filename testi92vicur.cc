...
//tests para las funciones setMonitor() y compruebaPassword() a partir de la linea 1096
...
//Pruebas para las funciones setMonitor y compruebaPassword

void testSetMonitor(){
	Administrativo adm("987654321", "787878")
	Monitor mon("123456789", "abc123", "Lucia", "Fernandez", "Perez", "600000000",
			"01/01/1990", "luciacorreo.com", "Calle Xyz numero 2 Cordoba");

	Monitor resultado;
	resultado = adm.setMonitor();

	ASSERT_EQUAL(mon.getDNI(),resultado.getDNI());
	ASSERT_EQUAL(mon.getNombre(),resultado.getNombre());
	ASSERT_EQUAL(mon.getApellidos(),resultado.getApellidos());
	ASSERT_EQUAL(mon.getTelefono(),resultado.getTelefono());
	ASSERT_EQUAL(mon.getFechaNacimiento(),resultado.getFechaNacimiento());
	ASSERT_EQUAL(mon.getCorreo(),resultado.getCorreo());
	ASSERT_EQUAL(mon.getDireccion(),resultado.getDireccion());
}

void testCompruebaPassword(){
	string dni = "dniprueba";
	string password = "12345K";
	Usuario usu(dni, password);
	bool resultado = usu.compruebaPassword();
	ASSERT(resultado == true);

}

void testComrpuebaPasswordIncorrecto(){
	string dni = "dniprueba";
	string password = "11345K";
	Usuario usu(dni, password);
	bool resultado = usu.compruebaPassword();
...
bool runAllTests(int argc, char const *argv[]) {
	...
	s.push_back(CUTE(testMonitor));
	s.push_back(CUTE(testCompruebaPassword));
	s.push_back(CUTE(testCompruebaPasswordIncorrecto));

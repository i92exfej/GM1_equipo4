...
//Test de la funcion setCliente de la clase administrativo
void setClienteTest(){

	Reserva aux;
	Administrativo a("00000000h","hola22");
	Cliente c("Antonio","Del Holmo Castejon","50505050X",954984,"Calle n5 Cordoba","Es menor de edad",aux);
	Cliente resultado;
	resultado = a.setCliente();

	//Asserts para valores correctos
	ASSERT_EQUAL(c.getNombre(),resultado.getNombre());
	ASSERT_EQUAL(c.getApellidos(),resultado.getApellidos());
	ASSERT_EQUAL(c.getDNI(),resultado.getDNI());
	ASSERT_EQUAL(c.getTelefono(),resultado.getTelefono());
	ASSERT_EQUAL(c.getDireccion(),resultado.getDireccion());
	ASSERT_EQUAL(c.getCondicion(),resultado.getCondicion());

	//Asserts para la comprobacion de la reserva (no se introduce en setCliente) por lo que puede provocar errores;
	//Se comprobara que efectivamente el atributo reserva esta vacio
	Reserva r;
	Reserva rc=resultado.getReserva();
	ASSERT_EQUAL(r.getRuta(),rc.getRuta());
	ASSERT_EQUAL(r.getCodigo(),rc.getCodigo());
	ASSERT_EQUAL(r.getFecha(),rc.getFecha());
}

//Test de la funcion setReserva de la clase administrativo
void setReservaTest(){
	Administrativo a("00000000h","hola22");
	Reserva aux;
	Cliente c("Antonio","Del Holmo Castejon","50505050X",954984,"Calle n5 Cordoba","Es menor de edad",aux);
	Ruta rut;
	rut.cambiarNombre("ruta1");
	parque.anyadirRuta(rut);
	Reserva R;
	Reserva ejemplo("ruta1","7 de octubre de 2020.","73Rj");
	a.setReserva(c);
	R=c.getReserva();
	ASSERT_EQUAL(ejemplo.getRuta(),R.getRuta());
	ASSERT_EQUAL(ejemplo.getCodigo(),R.getCodigo());
	ASSERT_EQUAL(ejemplo.getFecha(),R.getFecha());
}


//tests para la funcion mostrarRutas de la clase parque.
//El test se divide en tres tests, donde en cada uno se prueba una de las 3 opciones seleccionables en este metodo

//En el test1 se prueba la opcion de mostrar todas las rutas (opcion1)
void mostrarRutasOpc1(){
Ruta a, b, c;
Reserva r1("Ruta1","10 de octubre de 2020","578"), r2("Ruta1","11 de octubre de 2020","5748");
Parque P;
//cambios en atributos ruta1/a (usada para comprobar que la opc1 esta correcta)
a.cambiarNombre("Ruta1");
a.cambiarDuracionMinutos(54);
a.cambiarEnBici(true);
a.cambiarAPie(false);
a.cambiarAforo(3);
a.cambiarNivel("Dificil");
a.cambiarEstado("En mantenimiento");
//cambios en los atributos
a.anyadirReserva(r1);
a.anyadirReserva(r2);
//cambios en senderos
Sendero s1, s2;
s1.nombre="Primer sendero";
s2.nombre="Segundo sendero";
s1.duracion=s2.duracion=54;
s1.distancia=s2.distancia=7.4;
a.anyadirSendero(s1);
a.anyadirSendero(s2);

b.cambiarNombre("Ruta2");
c.cambiarNombre("Ruta3");

P.anyadirRuta(a);
P.anyadirRuta(b);
P.anyadirRuta(c);
cout<<"Test para la opcion 1"<<endl;
P.mostrarRutas();

ASSERT("true");
}

//test para probar la modificacion de los datos de una ruta (opcion 2)
void mostrarRutasOpc2(){
	Ruta a, b, c;
	Reserva r1("Ruta1","10 de octubre de 2020","578"), r2("Ruta1","11 de octubre de 2020","5748");
	Parque P;
	//cambios en atributos ruta1/a (usada para comprobar que la opc1 esta correcta)
	ASSERT_EQUAL(a.cambiarNombre("Ruta1"),true);
	ASSERT_EQUAL(a.cambiarDuracionMinutos(54),true);
	ASSERT_EQUAL(a.cambiarEnBici(true),true);
	ASSERT_EQUAL(a.cambiarAPie(false),true);
	ASSERT_EQUAL(a.cambiarAforo(3),true);
	ASSERT_EQUAL(a.cambiarNivel("Dificil"),true);
	ASSERT_EQUAL(a.cambiarEstado("En mantenimiento"),true);
	//cambios en los atributos
	ASSERT_EQUAL(a.anyadirReserva(r1),true);
	a.anyadirReserva(r2);
	//cambios en senderos (para las opciones 1 y 2)
	Sendero s1, s2;
	s1.nombre="Sendero1";
	s2.nombre="Sendero2";
	s1.duracion=s2.duracion=54;
	s1.distancia=s2.distancia=7.4;
	ASSERT_EQUAL(a.anyadirSendero(s1),true);
	a.anyadirSendero(s2);
	b.anyadirSendero(s1);
	b.anyadirSendero(s2);
	...
	ASSERT_EQUAL(P.anyadirRuta(a),true);
	P.anyadirRuta(b);
	P.anyadirRuta(c);

	cout<<"Test para la opcion 2"<<endl;
	P.mostrarRutas();
	P.mostrarRutas();

cout<<"La funcion muestra los datos de manera correcta tras modificarlos true(1)/false(0)?"<<endl;
bool opc;
cin>>opc;
if(opc){
	ASSERT(true);
...
//test para probar la modificacion de reservas dentro de una ruta (opcion3)
void mostrarRutasOpc3(){
	Ruta a;
	Reserva r1("Ruta1","10 de octubre de 2020","578"), r2("Ruta1","11 de octubre de 2020","5748");
	Parque P;
	//cambios en atributos ruta1/a (para comprobar que la opc1 esta correcta)
	ASSERT_EQUAL(a.cambiarNombre("Ruta1"),true);
	ASSERT_EQUAL(a.cambiarDuracionMinutos(54),true);
	ASSERT_EQUAL(a.cambiarEnBici(true),true);
	ASSERT_EQUAL(a.cambiarAPie(false),true);
	...
	ASSERT_EQUAL(a.cambiarNivel("Dificil"),true);
	ASSERT_EQUAL(a.cambiarEstado("En mantenimiento"),true);
	//añadir las reservas para modificarlas despues (para las opciones 1 y 3)
	ASSERT_EQUAL(a.anyadirReserva(r1),true);
	a.anyadirReserva(r2);

	ASSERT_EQUAL(P.anyadirRuta(a),true);

	cout<<"test para la opcion 3"<<endl;
	P.mostrarRutas();
	P.mostrarRutas();
...
bool runAllTests(int argc, char const *argv[]) {
	...
	s.push_back(CUTE(setClienteTest));
	s.push_back(CUTE(setReservaTest));
	s.push_back(CUTE(mostrarRutasOpc1));
	s.push_back(CUTE(mostrarRutasOpc2));
	s.push_back(CUTE(mostrarRutasOpc3));
